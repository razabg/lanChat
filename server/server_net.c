...
//all static the make the variables private.
...
int ServerNet_Create(int port, OnMessageCb on_msg, OnDisconnectCb on_disc)
{
    ...
    int opt = 1;
    setsockopt(s_listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));//keep the port reuseable
...
void ServerNet_Run(void)// add more sub funct
{
    s_running = 1;

    while (s_running) {

        /* --- Step 1: build the fd_set --- */
        fd_set read_fds;
        FD_ZERO(&read_fds);
        ...
        /* --- Step 4: check each client for data --- */
        itr = ListItrBegin(s_clients);
        end = ListItrEnd(s_clients);
        while (itr != end) {
            ListItr current = itr;
            itr = ListItrNext(itr);
            ...
            /* --- Step 5: disconnect --- */ //check when to dissconect client
            if (bytes <= 0) {
            ...
            /* --- Step 6: accumulate and check for complete TLV --- */
            node->buf_len += bytes;
            while (node->buf_len >= TLV_HEADER_SIZE) {
                uint16_t msg_len;
                memcpy(&msg_len, node->recv_buf + 1, 2);
