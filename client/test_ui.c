...
/*
 * Redirect stdin so the next UI call reads from the given string.
 * We keep a pointer to the previous tmpfile and close it before
 * opening a new one, preventing a file-descriptor leak.
 */
...
/* ── showMessage ──────────────────────────────────────────────────────── */
static void test_showMessage(void)
{
    printf("\n-- showMessage --\n");
    /* Visual test — just confirm it runs without crashing */
    showMessage("Registration successful!");
    showMessage("Wrong password.");
    CHECK(1, "showMessage runs without crash");
}
...
static void test_getMenuChoice(void)
{
    ...
    /* Bad input must return MENU_INVALID, not a random value */
    set_stdin("abc\n");
    CHECK(getMenuChoice() == MENU_INVALID,  "non-numeric input → MENU_INVALID");
}
...
static void test_getGroupMenuChoice(void)
{
    ...
    /* Bad input must return GROUP_MENU_INVALID, not a random value */
    set_stdin("xyz\n");
    CHECK(getGroupMenuChoice() == GROUP_MENU_INVALID, "non-numeric input → GROUP_MENU_INVALID");
}
...
static void test_getCredentials(void)
{
    ...
    /* strcspn must have stripped the newline */
